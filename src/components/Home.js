...
const Home = () => {
  ...
  const handleSearch = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}home/api/search/?query=${searchQuery}`, {
      ...
      if (response.ok) {
        ...
        console.error('Error fetching suggested information:', response.statusText);
      }
    } catch (error) {
    ...
    (async () => {
      try {
        ...
        if (response.ok) {
          ...
          console.error('Error fetching recommended papers:', response.statusText);
        }
      } catch (error) {
        console.error('An error occurred:', error);
      }
